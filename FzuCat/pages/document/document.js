...
Page({
  ...
  confirm: function () {//提交信息
    ...
    wx.request({
      ...
      data: {
        ...
        food: that.data.feedInfo.food+that.data.feedInfo.num,
        time: Date.toString,
        op: 1  //int
      },
      method: 'POST',
  ...
  addFeedDialog:function(confirm){
    var that = this
    var token = wx.getStorageSync('token')
    wx.request({
      url: 'https://iminx.cn/api/wxapp/feedCat/',
  ...
  food:function(e){
    var feedInfo = this.data.feedInfo
    feedInfo.food = e.detail.value
    this.setData({feedInfo:feedInfo})
  },
  num:function(e){
    var feedInfo = this.data.feedInfo
    feedInfo.num = e.detail.value
    this.setData({feedInfo:feedInfo})
  },
  gotoPost:function(){
    var that = this
    wx.navigateTo({
      ...
      success: function (res) {
        ...
        console.log("sent to document"+that.data.catId)
        res.eventChannel.emit('acceptDataFromOpenerPage', { data: that.data.catId })
      }
    })
  },
  /**
  ...
  onLoad: function (options) {
    ...
    this.checkIfLike(this.data.catId)
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
